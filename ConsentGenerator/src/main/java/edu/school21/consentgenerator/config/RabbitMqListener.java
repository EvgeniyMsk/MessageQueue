...
import edu.school21.consentgenerator.entity.Person;
...
public class RabbitMqListener {
    ...
    private void createGuaranteeLetter(Person person) throws DocumentException, IOException {
        ...
        if (!file.exists())
            file.mkdir();
        PdfWriter.getInstance(document, new FileOutputStream(String.format("%s%s%s/ConcentLetter.pdf",
                PATH,
                person.getLastName(),
                person.getName())));
        document.open();
        ...
        String main = String.format("Это Ваше ConcentLetter, %s %s!\n" +
                "Ваш возраст: %d\n" +

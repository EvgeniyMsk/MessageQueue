...
public class RabbitMqListener {
    ...
    public void processSocial_food(String message) throws DocumentException, IOException {
        ...
        createTransportationAgreement(person);
    }

    private void createTransportationAgreement(Person person) throws DocumentException, IOException {
        Document document = new Document();
        File file = new File(PATH + person.getLastName() + person.getName());
        if (!file.exists())
            file.mkdir();
        PdfWriter.getInstance(document, new FileOutputStream(String.format("%s%s%s/TransportationAgreement.pdf",
                PATH,
                person.getLastName(),
                person.getName())));
        document.open();
        ...
        String main = String.format("Это Ваш TransportationAgreement, %s %s!\n" +
                "Ваш возраст: %d\n" +
